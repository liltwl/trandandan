...
function Game() {
  ...
  React.useEffect(() => {
    socket.on('leftscored', () => {
      setLeftScore((prevScore: number) => {
        ...
        if (removeDecimalPart(newScore / 2) === 5) { //9alab 3la data lighay htaj khona o siftha lih
          setGameOver(true);
  ...
  const [gameMode, setGameMode] = React.useState<null | 'classic' | 'crazy' | 'IA'>(null);
  ...
  useEffect(() => {
    ...
    if (gameMode === null) {
      return (
        ...
        <button className='custom-button' onClick={() => setGameMode('classic')}>Classic</button>
        <button className='custom-button' onClick={() => setGameMode('crazy')}>Crazy</button>
        <button className='custom-button' onClick={() => setGameMode('IA')}>IA</button>
      </div>
    ...
    { !isGameReady &&
      (
          ...
          <p style={{ color: 'white', fontSize: '2em', textAlign: 'center', animation: 'fade 1.5s infinite' }}>
